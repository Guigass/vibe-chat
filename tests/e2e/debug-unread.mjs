...
// Bob opens a DM with Demo (switch away from #geral)
const demoDm = bob.page.getByText(/^Demo$/i).first();
await demoDm.click();
await bob.page.waitForTimeout(800);

console.log('Bob active channel heading:', await bob.page.locator('h1, [role="heading"]').first().textContent().catch(() => 'n/a'));

// Alice sends a message in #geral while Bob is viewing the DM
const composer = alice.page.locator('textarea').first();
const body = `debug-unread-${Date.now()}`;
...
const geralItem = await bob.page.evaluate(() => {
  ...
  return geral
    ? { text: geral.textContent?.trim(), html: geral.outerHTML }
    : { text: null, html: null, allButtons: buttons.map((b) => b.textContent?.trim()) };
});
console.log('BOB SIDEBAR GERAL ITEM AFTER ALICE MSG:', JSON.stringify(geralItem, null, 2));

await bob.page.screenshot({ path: 'debug-unread-bob.png' });
