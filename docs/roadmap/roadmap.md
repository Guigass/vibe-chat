...
**W10-7 / B-101 Done** (DM em grupo). **W10-8 / B-102 Done** (seguir thread:
auto-follow autor/resposta/menção na mesma transação; `threads/following` com não
lidas via `ConversationSequences - LastReadSeq`; `followAllThreads` por canal;
push trata seguidor como menção; compartilhar resposta no canal via forward
writer). **W10-9 / B-103 Done** (skip link, landmarks, foco em overlays, contraste
AA, anunciador resumido; axe-core como gate na CI). Próximo elegível: W10-10 / B-040.
...
| W7-10 | B/E | Filtro centralizado `RequirePermission` na API Minimal (B-174) | P2-1, B-041 | [B-174](../product/specs/B-174-filtro-centralizado-permissoes.md) | **Done** |
...
| ID | Trilha | Tarefa | Deps | Spec | Status |
|----|--------|--------|------|------|--------|
| W9-0 | D | Message bubble moderno — layout tipado, preview e context menu (B-163) | B-023, B-024, B-083 | [B-163](../product/specs/B-163-message-bubble-context-menu.md) | **Done** |
...
| W9-7 | C/D | Recibos de leitura e não lidas persistentes (B-094) | W9-1 | [B-094](../product/specs/B-094-recibos-de-leitura.md) | **Done** |
...
| W10-9 | D/E | Acessibilidade WCAG 2.2 AA (B-103) | W10-5 | [B-103](../product/specs/B-103-acessibilidade.md) | **Done** |
...
| `GAP-ci-duplicate-runs` | A | CI em `cursor/**` + `pull_request` duplicava workflows; push só em `main` + `concurrency` cancel-in-progress | #149 |
