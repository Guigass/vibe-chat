...
| B-187 | Instalação configurável no admin (integração no DB) | Planned (W7-14) — SMTP/IA/webhook/retenção/Files/RateLimit na UI quando o keyring for válido; `.env` **mantém** infra/portas/pins; **não** dump no DB (D-04 / ADR-020); [spec](../product/specs/B-187-env-enxuto.md) |
