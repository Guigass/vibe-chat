...
| B-187 | Instalação configurável no admin (integração no DB) | Planned (W7-14) — `.env` **só infra**; produto (SMTP/IA/webhook/retenção/Files/RateLimit/link preview/VAPID/kill switches) na UI quando overrides on (D-04 / ADR-020); [spec](../product/specs/B-187-env-enxuto.md) |
...
avançar feature de wave; **W7-14 / B-187** (instalação configurável no admin —
integração no DB, infra no env) enquanto
