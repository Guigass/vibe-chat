...
| B-105 | Catálogo de configuração admin mínima no `.env` | **Done (W7-7)** — EMAIL__* injetado no api; retenção no worker; alias OpenRouter unificado; testes `ComposeConfigCatalogTests`; [spec](../product/specs/B-105-catalogo-configuracao.md). Follow-up: **B-187** (integração no admin; infra no env) |
| B-187 | Instalação configurável no admin (integração no DB) | **Done (W7-14)** — `.env` só infra; produto (SMTP/IA/webhook/retenção/Files/RateLimit/link preview/VAPID/kill switches) na UI quando overrides on (D-04 / ADR-020); [spec](../product/specs/B-187-env-enxuto.md) |
...
| B-103 | Acessibilidade WCAG 2.2 AA | Planned (W10-9) — implementação local validada em 2026-09-10; 293 unitários e 4 E2E verdes; aguarda PR/CI/merge; [evidência](../product/specs/B-103-acessibilidade.md) |
