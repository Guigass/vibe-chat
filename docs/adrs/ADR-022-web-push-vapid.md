...
2. **VAPID no env** (`Push:Vapid:PublicKey|PrivateKey|Subject`) — secret D-04;
   a API só devolve a chave **pública**.
...
6. **Kill switch** `Push:Enabled` (default `false`). Sem chaves VAPID válidas o
   sender é no-op. Falha de push **não** reprocessa o outbox.
