...
| **Configuração sensível** | Tokens, webhooks, chaves de AI/SMTP e secrets de integração. Só administradores leem/alteram via `/admin` + `GET/PUT /api/v1/admin/settings` (`workspace.admin`); membros/Auditor → 403; resposta só com máscara/`configured` (B-069). Secrets de **infra** (DB, IdP, MinIO, keyring, VAPID) ficam só em env/secret store (D-04). Credenciais de **integração** (OpenRouter, senha SMTP, HMAC de webhook) podem ir ao DB em envelope AES-GCM quando `RuntimeSettings:DatabaseOverridesEnabled` e o keyring for válido (ADR-020). B-187 fecha esse caminho na instalação nova; infra permanece no `.env`. |
