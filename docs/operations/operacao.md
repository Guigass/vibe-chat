...
- Reimport controlado do Keycloak: mudar `KEYCLOAK_DB_SCHEMA` para um schema novo aciona o initializer idempotente `keycloak-schema` e recria o estado do Keycloak a partir do realm versionado sem apagar o schema anterior; sessões e usuários anteriores deixam de valer.
