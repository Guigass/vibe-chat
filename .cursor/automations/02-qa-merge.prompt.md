...
6. **Track tests** — run suites for changed modules (`task test`, targeted projects)
...
1. Post a **PR comment** using the output format below.
...
3. On flake unrelated to the change: say so and re-run; do not hide real failures.

### Accept / merge policy

| Verdict | Action |
|---------|--------|
| **PASS** | Approve the PR. Enable GitHub **auto-merge** (squash) if available; if the environment allows a merge command and CI is green + branch rules satisfied, merge. |
